...
void test_trdos_scl_and_beta() {
    ...
    // Geometry comes from the disk-info type byte, so 320 KB images are not
    // guessed wrong and short images (truncated after the last used sector) load.
    auto make_trd = [](uint8_t type, size_t size) {
        std::vector<uint8_t> image(size, 0);
        image[8 * 256 + 0xe3] = type;
        return image;
    };
    struct GeometryCase {
        uint8_t type;
    ...
    const GeometryCase cases[] = {
        {0x16, 80, 2, "TRD type $16 is 80 tracks double sided"},
        {0x17, 40, 2, "TRD type $17 is 40 tracks double sided"},
        {0x18, 80, 1, "TRD type $18 is 80 tracks single sided"},
        {0x19, 40, 1, "TRD type $19 is 40 tracks single sided"},
    };
    for (const GeometryCase& c : cases) {
        dsp::TrdosDisk geo;
        const std::vector<uint8_t> image = make_trd(c.type, 40 * 16 * 256);
        check(geo.load_bytes(image.data(), image.size(), &error), "geometry image loads");
        check(geo.tracks() == c.tracks && geo.heads() == c.heads, c.name);
    }

    dsp::TrdosDisk short_disk;
    const std::vector<uint8_t> short_image = make_trd(0x16, 0x900);
    check(short_disk.load_bytes(short_image.data(), short_image.size(), &error),
          "a TRD truncated after the catalogue still loads");
    check(short_disk.tracks() == 80 && short_disk.heads() == 2,
          "the truncated TRD keeps its DS/80 geometry");
    check(short_disk.sector(79, 1, 16) != nullptr,
          "sectors past the end of a truncated TRD are padded with zeros");

    // Port $FF bit 4 selects the head with inverted polarity: TR-DOS clears it
    // to read side 1, so a straight copy of the bit reads the wrong side.
    const fs::path trd_path = dir / "sides.trd";
    {
        std::vector<uint8_t> image = make_trd(0x16, 80 * 2 * 16 * 256);
        image[0] = 0x11;              // cylinder 0, head 0, sector 1
        image[16 * 256 + 0] = 0x22;   // cylinder 0, head 1, sector 1
        std::ofstream out(trd_path.string(), std::ios::binary);
        out.write(reinterpret_cast<const char*>(image.data()), std::streamsize(image.size()));
    }
    dsp::Beta128 sides;
    sides.reset();
    check(sides.load_disk(trd_path.string(), &error), "Beta 128 mounts a TRD image");
    sides.enable();
    auto read_first_byte = [&](uint8_t latch) {
        sides.param_w(latch);
        sides.track_w(0);
        sides.sector_w(1);
        sides.command_w(0x80);
        return sides.data_r();
    };
    check(read_first_byte(0x3c) == 0x11, "port $FF bit 4 set selects side 0");
    check(read_first_byte(0x2c) == 0x22, "port $FF bit 4 clear selects side 1");
    sides.param_w(0x3f);  // drive B, which has no disk
    check((sides.status_r() & 0x80) != 0, "selecting an empty drive reports NOT READY");

    dsp::Wd1793 addr_fdc;
    dsp::TrdosDisk addr_disk;
    const std::vector<uint8_t> addr_image = make_trd(0x16, 80 * 2 * 16 * 256);
    check(addr_disk.load_bytes(addr_image.data(), addr_image.size(), &error), "address image loads");
    addr_fdc.reset();
    addr_fdc.set_disk(&addr_disk);
    addr_fdc.data_w(5);
    addr_fdc.command_w(0x18);  // seek
    check(addr_fdc.track_r() == 5, "seek moves the head to the track in the data register");
    addr_fdc.command_w(0xc0);  // read address
    check(addr_fdc.data_r() == 5, "read address returns the current track");
    check(addr_fdc.sector_r() == 5, "read address copies the track into the sector register");
}
