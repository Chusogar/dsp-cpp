...
void SegaSystem1::on_sound_cycles(int cycles) {
    ...
    while (audio_accumulator_ >= int64_t(kSoundClock)) {
        ...
        // SN76496 amplitude set in ctor (0.5f / 1.0f) like Pascal Create()
        const int32_t sample = psg0_.update() + psg1_.update();
...
void SegaSystem1::run_frame() {
    for (int line = 0; line < kScanlines; line++) {
        ...
        // Sound IRQ every 64 scanlines (system1_sound_irq in system1_hw.pas).
        ++sound_irq_accum_;
        if (sound_irq_accum_ >= 64) {
            sound_irq_accum_ = 0;
