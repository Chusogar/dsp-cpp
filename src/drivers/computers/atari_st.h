...
public:
    ...
    uint8_t peek(uint32_t address) const {
        try {
            return const_cast<AtariSt*>(this)->read_byte(address);
        } catch (const M68000::BusError&) {
            return 0xff;
        }
    }
    ...
    void poke_word(uint32_t address, uint16_t value) {
        try {
...
private:
    ...
    void service_acia();
    static void blitter_access(uint32_t address);
