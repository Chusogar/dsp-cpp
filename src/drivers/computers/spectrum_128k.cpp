...
Spectrum128k::Spectrum128k(Model model)
    : model_(model), cpu_(kClock), ay0_(kAyClock), ay1_(kAyClock) {
    for (int i = 0; i < 16; ++i) {
        palette_[i] = to_argb(kPalette[i] | 0xff000000);
        palette_ext_[i] = palette_[i];
    }
    for (int i = 16; i < 80; ++i) palette_ext_[i] = 0xff000000;
}

const char* Spectrum128k::title() const {
    return model_ == Model::SpecPlus2 ? "ZX Spectrum +2" : "ZX Spectrum 128K";
}

uint32_t Spectrum128k::ulaplus_decode(uint8_t value) const {
...
void Spectrum128k::ulaplus_set_entry(uint8_t index, uint8_t value) {
    if (index >= 64) return;
...
void Spectrum128k::build_contention() {
    contention_.fill(0);
...
void Spectrum128k::apply_7ffd(uint8_t value) {
    const uint8_t new_pant = uint8_t(((value & 8) >> 2) + 5);  // 5 or 7
    pantalla_ = new_pant;
    port_7ffd_ = value;
    if (paging_enabled_) {
        paging_enabled_ = (value & 0x20) == 0;
        marco_[0] = uint8_t(((value >> 4) & 1) + 8);  // ROM 8 or 9
        marco_[3] = value & 7;                        // RAM bank at $C000
    }
}
...
void Spectrum128k::set_dip_switch(int, uint8_t) {}
void Spectrum128k::set_inputs(const MachineInputs& inputs) { apply_keyboard(inputs); }

void Spectrum128k::apply_keyboard(const MachineInputs& in) {
    keys_.fill(0xff);
...
void Spectrum128k::mem_write(uint16_t addr, uint8_t value) {
    const int slot = addr >> 14;
    const uint8_t bank = marco_[slot];
...
void Spectrum128k::drain_audio(std::vector<int16_t>& out) {
    out.swap(audio_);
    audio_.clear();
}

bool Spectrum128k::load_media(const std::string& path, std::string* error) {
...
bool Spectrum128k::load_if2(const std::string& path, std::string* error) {
    std::vector<uint8_t> buf;
    if (!load_file(path, buf)) {
