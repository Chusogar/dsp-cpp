...
rte_loop:
		...
		switch(format_word)
		{
			...
			case 2: /* Trap */
				...
				CPU_RUN_MODE = RUN_MODE_NORMAL;
				return;
			case 0xa: /* Short bus cycle fault (16 words) */
			case 0xb: /* Long bus cycle fault (46 words) */
			{
				/* dsp-cpp: the faulting instruction is restarted from the
				   stacked PC (registers were rolled back when the fault was
				   taken), which is what re-running the bus cycle achieves. */
				int words = (format_word == 0xa) ? 16 : 46;
				int i;
				new_sr = m68ki_pull_16();
				new_pc = m68ki_pull_32();
				for (i = 3; i < words; i++)
