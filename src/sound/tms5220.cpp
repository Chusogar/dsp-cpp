...
void Tms5220::apply_rs_ws(bool nw,bool nr){const bool ow=wsq_,orq=rsq_;if(ow==nw&&orq==nr)return;wsq_=nw;rsq_=nr;if(!nw&&!nr){chip_reset();return;}if(ow&&!nw&&nr){if(data_pending_){data_pending_=false;write_data(data_latch_);}ready_delay_=16;}if(orq&&!nr&&nw){rs_read_=true;ready_delay_=16;raise_irq(false);}}
