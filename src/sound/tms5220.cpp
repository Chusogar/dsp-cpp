...
inline int coeff(int n, int index) {
    ...
    return kK[n][std::clamp(index, 0, max)];
}
...
// TMS5220 analog speaker output: 14-bit lattice value -> 8-bit DAC with
// the non-linear clipping/range-extension documented by the original chip.
...
void Tms5220::chip_reset() {
    ...
    old_k_idx_.fill(0);
    new_k_idx_.fill(0);
    current_energy_ = previous_energy_ = current_pitch_ = 0;
    current_k_.fill(0);
    ip_ = pc_ = 0;
    subcycle_ = 1;
    pitch_count_ = 0;
    ...
    old_silence_ = true;
    zpar_ = uv_zpar_ = true;
    frame_just_parsed_ = false;
    rng_ = 0x1fff; // TMS5220 power-on/reset LFSR state used by MAME.
    u_.fill(0);
    x_.fill(0);
    out_sample_ = 0;
    cycle_acc_ = 0;
...
void Tms5220::process_command(uint8_t cmd) {
    ...
    case 0x60: // SPEAK EXTERNAL
        ...
        new_energy_idx_ = new_pitch_idx_ = 0;
        new_k_idx_.fill(0);
        update_fifo_flags(false);
        raise_irq(false);
        break;
    case 0x70: // RESET
        chip_reset();
...
void Tms5220::write_data(uint8_t value) {
    ...
    fifo_[fifo_tail_] = value;
    fifo_tail_ = (fifo_tail_ + 1) & 15;
    ++fifo_count_;
    buffer_low_ = fifo_count_ <= 8;
    buffer_empty_ = false;

    // SPEAK EXTERNAL starts on the BL falling edge: the ninth byte makes
    // FIFO count 9 and therefore clears BL.
...
uint32_t Tms5220::extract_bits(int n) {
    ...
    while (n--) {
        if (bits_left_ == 0) {
            ...
            bits_left_ = 8;
            update_fifo_flags(true);
        }
        v = (v << 1) | (bit_buffer_ & 1u);
        bit_buffer_ >>= 1;
        --bits_left_;
    }
    return v;
...
bool Tms5220::parse_frame() {
    ...
    if (repeat) {
        new_k_idx_ = old_k_idx_;
        return true;
    }
    for (int i = 0; i < 4; ++i) new_k_idx_[i] = int(extract_bits(kKBits[i]));
    if (new_pitch_idx_ == 0) {
        for (int i = 4; i < 10; ++i) new_k_idx_[i] = 0;
    } else {
        for (int i = 4; i < 10; ++i) new_k_idx_[i] = int(extract_bits(kKBits[i]));
    }
    return true;
}

int16_t Tms5220::lattice(int16_t excitation) {
    // Exact MAME/TMS5220 lattice ordering. The previous implementation used
    // K9/K10 against the wrong delay elements, which severely distorted speech.
    u_[10] = mul(previous_energy_, int32_t(excitation) << 6);
    u_[9] = u_[10] - mul(current_k_[9], x_[9]);
    u_[8] = u_[9] - mul(current_k_[8], x_[8]);
    u_[7] = u_[8] - mul(current_k_[7], x_[7]);
    u_[6] = u_[7] - mul(current_k_[6], x_[6]);
    u_[5] = u_[6] - mul(current_k_[5], x_[5]);
    u_[4] = u_[5] - mul(current_k_[4], x_[4]);
    u_[3] = u_[4] - mul(current_k_[3], x_[3]);
    u_[2] = u_[3] - mul(current_k_[2], x_[2]);
    u_[1] = u_[2] - mul(current_k_[1], x_[1]);
    u_[0] = u_[1] - mul(current_k_[0], x_[0]);

    // x10 is not part of the stored state. The real recurrence updates x9
    // using K9 and u9, x8 using K8/u8, ... x1 using K1/u1.
    x_[9] = x_[8] + mul(current_k_[8], u_[8]);
    x_[8] = x_[7] + mul(current_k_[7], u_[7]);
    x_[7] = x_[6] + mul(current_k_[6], u_[6]);
    x_[6] = x_[5] + mul(current_k_[5], u_[5]);
    x_[5] = x_[4] + mul(current_k_[4], u_[4]);
    x_[4] = x_[3] + mul(current_k_[3], u_[3]);
    x_[3] = x_[2] + mul(current_k_[2], u_[2]);
    x_[2] = x_[1] + mul(current_k_[1], u_[1]);
    x_[1] = x_[0] + mul(current_k_[0], u_[0]);
    x_[0] = u_[0];
...
void Tms5220::generate_sample() {
    ...
    if (ip_ == 0 && pc_ == 12 && subcycle_ == 1) {
        if (!parse_frame()) {
            // MAME keeps TALKD alive long enough to ramp the final frame down.
            // For this compact core, force the synthesis parameters to zero
            // and retain a short analog decay instead of a hard discontinuity.
            current_energy_ = 0;
            current_pitch_ = 0;
            current_k_.fill(0);
            talk_status_ = false;
        ...
        inhibit_ = (old_unvoiced_ != nu) || (old_silence_ && !ns) || (old_unvoiced_ && ns);
        zpar_ = ns;
        uv_zpar_ = nu;
    }
    ...
    if (subcycle_ == 2 && pc_ <= 11) {
        ...
        auto interp = [&](int cur, int target) {
            if (!allow) return cur;
            if (sh == 0) return target;
            return cur + ((target - cur) >> sh);
        };
        if (pc_ == 0) {
            current_energy_ = zpar_ ? 0 : interp(current_energy_, kEnergy[new_energy_idx_]);
        } else if (pc_ == 1) {
            current_pitch_ = zpar_ ? 0 : interp(current_pitch_, kPitch[new_pitch_idx_]);
        } else {
            const int k = pc_ - 2;
    ...
    int16_t excitation;
    if (old_unvoiced_) {
        // The LFSR advances 20 times per 8-kHz output sample (one time per
        // internal T-cycle). Using one shift per sample makes the noise highly
        // correlated and was a major source of the dirty/robotic output.
        for (int i = 0; i < 20; ++i) {
            const uint16_t bit = uint16_t(((rng_ >> 12) ^ (rng_ >> 3) ^ (rng_ >> 2) ^ rng_) & 1);
            rng_ = uint16_t((rng_ << 1) | bit);
        }
        excitation = (rng_ & 1) ? int16_t(-0x3f) : int16_t(0x40);
    } else {
    ...
    ++pitch_count_;
    if (current_pitch_ == 0 || pitch_count_ >= current_pitch_ || (inhibit_ && ip_ == 0)) pitch_count_ = 0;
    pitch_count_ &= 0x1ff;

    // PC 12 has no B cycle. PC 0..11 have A+B, so a complete normal frame is
    // 8 * (13 A + 12 B) = 200 samples at the 8-kHz synthesis rate.
    ++subcycle_;
    if (pc_ == 12 && subcycle_ == 2) {
        subcycle_ = 1;
        pc_ = 0;
        ip_ = (ip_ + 1) & 7;
        if (ip_ == 0) {
