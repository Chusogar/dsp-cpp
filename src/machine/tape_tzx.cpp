                ...
                if (b.totp != 0) {
                    ...
                    for (uint16_t s = 0; s < nsym; ++s) {
                        ...
                        for (uint8_t p = 0; p < b.npp; ++p) {
                            ...
                            pos += 2;
                            // Zero length = end of this symbol's pulse list (Fuse behaviour)
                            if (v == 0) {
                                // still consume remaining slots
                                for (uint8_t q = uint8_t(p + 1); q < b.npp; ++q) {
                                    pos += 2;
                                }
                                break;
                            }
                            sym.pulses.push_back(v);
                        }
                        b.pilot_symbols.push_back(std::move(sym));
                    }
                    // Pilot stream: TOTP entries of (symbol index u8, repeat u16)
                    // Expand into flat pulse train (dsp-emulator / Pascal style).
                    for (uint32_t i = 0; i < b.totp; ++i) {
                        ...
                        if (sym_i < b.pilot_symbols.size()) {
                            const Symbol& sym = b.pilot_symbols[sym_i];
                            // Store flag of first stream symbol as initial_level
                            if (i == 0) b.initial_level = sym.flags;
                            for (uint16_t r = 0; r < rep; ++r) {
                                for (uint16_t d : sym.pulses) {
                                    if (d) b.pulses.push_back(d);
                                }
                            }
                        }
                    }
                ...
                if (b.totd != 0) {
                    ...
                    for (uint16_t s = 0; s < nsym; ++s) {
                        ...
                        for (uint8_t p = 0; p < b.npd; ++p) {
                            ...
                            pos += 2;
                            if (v == 0) {
                                for (uint8_t q = uint8_t(p + 1); q < b.npd; ++q) pos += 2;
                                break;
                            }
                            sym.pulses.push_back(v);
                        }
...
void TapeTzx::start_gen_symbol(const Symbol& sym, bool apply_flags) {
    // dsp-emulator: flags applied at block/byte start, not every intra-byte symbol.
    //   0 = keep, 1 = toggle, 2 = force low, 3 = force high
    if (apply_flags) {
        switch (sym.flags & 3) {
            case 0: break;
            case 1: level_ ^= 0x40; break;
            case 2: level_ = 0; break;
            case 3: level_ = 0x40; break;
        }
    }
    sym_pulse_i_ = 0;
    if (sym.pulses.empty()) {
    ...
    estados_left_ = int(sym.pulses[0] ? sym.pulses[0] : 1);
}
...
void TapeTzx::advance_gen_data(bool apply_flags) {
    ...
    start_gen_symbol(b.symbols[sym_i], apply_flags);
    estado_ = 20;
}
...
void TapeTzx::start_block() {
    ...
    pulse_index_ = 0;
    bit_mask_ = 0x80;
    // used_bits applies ONLY to the last byte. Intermediate bytes always use all 8 bits.
    last_bit_mask_ = 1;
    if (b.data.size() == 1) {
        uint8_t ub = b.used_bits ? b.used_bits : 8;
        if (ub < 8) last_bit_mask_ = uint8_t(1u << (8 - ub));
    }
    sym_pulse_i_ = 0;
    pilot_stream_i_ = 0;
    ...
    switch (b.type) {
        ...
        case BlockType::Generalized:
            // Prefer pre-expanded pilot pulse train (Pascal $13 style)
            if (!b.pulses.empty()) {
                // Apply first symbol flags once (Pascal initial_val)
                const uint8_t pol = b.initial_level & 3;
                switch (pol) {
                    case 0: break;
                    case 1: level_ ^= 0x40; break;
                    case 2: level_ = 0; break;
                ...
                pulse_index_ = 0;
                estado_ = 10;  // pulse sequence
                estados_left_ = int(b.pulses[0] ? b.pulses[0] : 1);
            } else if (b.totp && !b.pilot_stream.empty() && !b.pilot_symbols.empty()) {
                ...
                if (si < b.pilot_symbols.size()) {
                    current_symbol_ = si;
                    start_gen_symbol(b.pilot_symbols[si], true);
                    estado_ = 19;
                } else {
                    advance_gen_data();
                }
...
int TapeTzx::advance(int tstates) {
    ...
    while (tstates > 0 && playing_) {
        ...
        switch (estado_) {
            ...
            case 4: {
                ...
                if (data_pos_ < b.data.size()) {
                    ...
                    last_bit_mask_ = (data_pos_ + 1 == b.data.size())
                                         ? uint8_t(1u << (8 - (b.used_bits ? b.used_bits : 8)))
                                         : uint8_t(1);
                    estado_ = 3;
            ...
            case 10:  // pulse sequence (also expanded $19 pilot)
                ...
                if (pulse_index_ < b.pulses.size()) {
                    uint32_t d = b.pulses[pulse_index_];
                    if (d == 0) d = 1; // never 0 — avoids infinite loop in advance()
                    estados_left_ = int(d > 0x7fffffffu ? 0x7fffffff : d);
                } else if (b.type == BlockType::Generalized && b.totd) {
                    data_pos_ = 0;
                    advance_gen_data();
                } else {
                    next_block();
            ...
            case 19: {  // generalized pilot symbol pulses (Pascal-compatible)
                ...
                ++sym_pulse_i_;
                if (sym_pulse_i_ < sym->pulses.size() && sym->pulses[sym_pulse_i_] != 0) {
                    // next pulse of same symbol occurrence: always toggle
                    level_ ^= 0x40;
                    estados_left_ = int(sym->pulses[sym_pulse_i_] ? sym->pulses[sym_pulse_i_] : 1);
                } else {
                    // end of one symbol occurrence
                    if (pilot_repeat_left_ > 1) {
                        --pilot_repeat_left_;
                        // Pascal expands repeats into ONE continuous train:
                        // do NOT re-apply force flags; just toggle into next first pulse
                        sym_pulse_i_ = 0;
                        level_ ^= 0x40;
                        estados_left_ = int(sym->pulses[0] ? sym->pulses[0] : 1);
                    } else {
                        // next pilot_stream entry — apply that symbol's flags
                        pilot_stream_i_ += 2;
                        if (pilot_stream_i_ + 1 < b.pilot_stream.size()) {
                            const uint8_t si = uint8_t(b.pilot_stream[pilot_stream_i_]);
                            pilot_repeat_left_ = b.pilot_stream[pilot_stream_i_ + 1];
            ...
            case 20: {  // generalized data — Pascal-compatible bit stream
                const Symbol* sym = nullptr;
                ...
                if (need_next) {
                    // end of symbol: always xor (Pascal)
                    level_ ^= 0x40;
                    ++data_pos_;
                    if (data_pos_ >= b.totd || b.symbols.empty()) {
                        advance_gen_data(true);
                        break;
                    }
                    const uint16_t asd = alphabet_size(uint8_t(b.asd));
                    bool apply = true;
                    if (asd <= 2) {
                        // Flags only when entering a new byte (Pascal bit_actual)
                        if (bit_mask_ > 1) {
                            bit_mask_ >>= 1;
                            apply = false;
                        } else {
        ...
        // Guard: zero duration would spin forever in this while
        if (estados_left_ <= 0) estados_left_ = 1;
    }
