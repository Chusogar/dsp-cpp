...
void Mos6566::reset() {
    linea_ = irq_raster_ = 0;
    ctrl1_ = ctrl2_ = irq_flag_ = irq_mask_ = 0;
    me_ = mxe_ = mye_ = mdp_ = mmc_ = 0;
    ec_ = b0c_ = b1c_ = b2c_ = b3c_ = 0;
    mm0_ = mm1_ = 0;
    mx_.fill(0); my_.fill(0); sc_.fill(0);
    mx8_ = vbase_ = 0;
    cia_va_ = 0;
    vc_ = vc_base_ = rc_ = 0;
    bad_lines_ = false;
    clx_spr_ = clx_bgr_ = 0;
    spr_coll_.fill(0); fore_mask_.fill(0);
}
...
uint8_t Mos6566::read(uint8_t reg) {
    switch (reg & 0x3F) {
        ...
        case 0x08: case 0x0A: case 0x0C: case 0x0E: return mx_[(reg & 0x0E) >> 1];
        case 0x01: case 0x03: case 0x05: case 0x07:
        case 0x09: case 0x0B: case 0x0D: case 0x0F: return my_[reg >> 1];
        case 0x10: return mx8_;
        case 0x11: return uint8_t((ctrl1_ & 0x7F) | ((linea_ & 0x100) >> 1));
        case 0x12: return uint8_t(linea_ & 0xFF);
        case 0x15: return me_;
        case 0x16: return uint8_t(ctrl2_ | 0xC0);
        case 0x17: return mye_;
        case 0x18: return uint8_t(vbase_ | 1);
        case 0x19: return uint8_t(irq_flag_ | 0x70);
        case 0x1A: return uint8_t(irq_mask_ | 0xF0);
        case 0x1B: return mdp_;
        case 0x1C: return mmc_;
        case 0x1D: return mxe_;
        case 0x1E: { uint8_t v = clx_spr_; clx_spr_ = 0; return v; }
        case 0x1F: { uint8_t v = clx_bgr_; clx_bgr_ = 0; return v; }
        ...
        case 0x2B: case 0x2C: case 0x2D: case 0x2E: return uint8_t(sc_[reg - 0x27] | 0xF0);
        default: return 0xFF;
    }
}
...
void Mos6566::write(uint8_t reg, uint8_t value) {
    switch (reg & 0x3F) {
        case 0x00: case 0x02: case 0x04: case 0x06:
        case 0x08: case 0x0A: case 0x0C: case 0x0E: mx_[(reg & 0x0E) >> 1] = value; break;
        case 0x01: case 0x03: case 0x05: case 0x07:
        case 0x09: case 0x0B: case 0x0D: case 0x0F: my_[reg >> 1] = value; break;
        case 0x10: mx8_ = value; break;
        case 0x11: {
            ctrl1_ = value;
            const uint16_t r = uint16_t((irq_raster_ & 0xFF) | ((value & 0x80) << 1));
            if (r != irq_raster_ && linea_ == r) raster_irq();
            irq_raster_ = r;
            break;
        }
        case 0x12: {
            const uint16_t r = uint16_t((irq_raster_ & 0x100) | value);
            if (r != irq_raster_ && linea_ == r) raster_irq();
            irq_raster_ = r;
            break;
        }
        ...
        case 0x27: case 0x28: case 0x29: case 0x2A:
        case 0x2B: case 0x2C: case 0x2D: case 0x2E: sc_[reg - 0x27] = value & 15; break;
        default: break;
    }
}
...
int Mos6566::update_line(int line, uint32_t* fb_row) {
    ...
    // Keep the VIC video matrix phase tied to the PAL frame.  VCBASE is the
    // base fetched at each badline; the renderer below deliberately uses the
    // absolute 40x25 matrix offset so that a badline phase cannot double-add
    // the row offset and make BASIC text jump to another screen row.
    if (line == 0x30) {
        vc_base_ = vc_ = rc_ = 0;
        bad_lines_ = false;
    }
    ...
    // PAL 6569 active character area is raster 51..250.  Do not derive the
    // visible character row from VCBASE: the latter is a bus-fetch counter,
    // while the framebuffer is an already rasterized 25-row image.
    ...
    for (int col = 0; col < 40; ++col) {
        // In text mode this is the actual screen matrix location.  The old
        // implementation added vc_base_ here as well as char_row*40, so
        // after every badline the same screen was effectively advanced twice.
        ...
        uint8_t pixels;
        if (bmm) {
            pixels = vic_read(uint16_t(bitmap_base + (vc << 3) + pixel_row));
        } else {
            const uint16_t ca = uint16_t(char_base + ((ch & (ecm ? 0x3F : 0xFF)) << 3) + pixel_row);
            pixels = vic_read(ca);
        }
        ...
        for (int px = 0; px < 8; ++px) {
            ...
            uint8_t color = b0c_;
            if (bmm) {
                color = (pixels & (0x80 >> px)) ? uint8_t(ch >> 4) : uint8_t(ch & 0x0F);
            } else if (mcm && (colc & 8)) {
                const int pair = (pixels >> (6 - (px & ~1))) & 3;
                color = pair == 0 ? b0c_ : pair == 1 ? b1c_ : pair == 2 ? b2c_ : uint8_t(colc & 7);
    ...
    // Sprite rendering remains supported, but is intentionally performed
    // after the stable character matrix so sprites cannot alter the BASIC
    // text raster timing.
    spr_coll_.fill(0);
    ...
    for (int s = 7; s >= 0; --s) {
        ...
        const uint16_t sp_ptr_base = uint16_t(video_base + 0x3F8);
        ...
        for (int byte = 0; byte < 3; ++byte) {
            ...
            for (int b = 0; b < 8;) {
                ...
                for (int px = 0; px < width * xexp; ++px) {
                    ...
                    if (sx < 0 || sx >= kScreenWidth) continue;
                    if (spr_coll_[sx]) clx_spr_ = uint8_t(clx_spr_ | spr_coll_[sx] | (1 << s));
                    spr_coll_[sx] = uint8_t(spr_coll_[sx] | (1 << s));
                    if (fore_mask_[sx]) clx_bgr_ = uint8_t(clx_bgr_ | (1 << s));
                    if (!(behind && fore_mask_[sx])) fb_row[sx] = kPalette[color & 15];
                }
            }
        }
    }
