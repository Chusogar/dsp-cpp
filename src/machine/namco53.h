...
// Namco 53xx: MB8843 MCU used as I/O (steering + DIPs on Pole Position).
class Namco53xx {
public:
    using PortRead = std::function<uint8_t()>;
    using PortWrite = std::function<void(uint8_t)>;

    explicit Namco53xx(uint32_t clock);

    bool load_rom(const std::vector<uint8_t>& rom, std::string* error);
    void set_k_read(PortRead handler) { read_k_ = std::move(handler); }
