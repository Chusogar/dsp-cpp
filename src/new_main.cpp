...
void print_usage(const char* program) {
    ...
    std::printf(
        ...
        "                     Atari ST .st/.msa, Amiga .adf, Macintosh 400K/800K/1.44MB SuperDrive .dsk/.img/.dc42,\n"
...
int main(int argc, char** argv) {
    ...
    // SAM Coupe: after inserting a disk, let the ROM finish cold-start
    // (system variables / free-page table) then invoke BOOT. Jumping at
    // D8DF immediately left BOOTEX unable to find a free page.
    if (any_media && game == "samcoupe") {
        if (auto* sam = dynamic_cast<dsp::SamCoupe*>(machine.get())) {
            sam->request_boot();
            std::fprintf(stderr, "samcoupe: disk loaded, BOOT scheduled after ROM init\n");
        }
    }
